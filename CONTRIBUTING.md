...
* Open an Issue at https://github.com/standup-raven/standup-raven/issues
...
* If you or the maintainers don't respond to an issue for 30 days, the [issue will be closed](#clean-up-issues-and-prs). If you want to come back to it, reply (once, please), and we'll reopen the existing issue. Please avoid filing new issues as extensions of ones you already made.
...
* The project team will [label the issue](#label-issues).
* A team member will try to reproduce the issue with the steps you provide. If there are no repro steps or no obvious way to reproduce the issue, the team will ask you for those steps and mark the issue as `needs-repro`. Bugs with the `needs-repro` tag will not be addressed until they are reproduced.
...
* Open an issue at https://github.com/standup-raven/standup-raven/issues
* Provide as much context as you can about what you're running into.
* Please try to be clear about why existing features and alternatives would not work for you.
...
Helping other users with their questions is a really awesome way of contributing to any community. It's not uncommon for most of the issues on an open source project to be support-related questions by users trying to understand something they ran into, or find their way around a known bug.
...
* Go to the issue tracker and [filter open issues by the `support` label](https://github.com/standup-raven/standup-raven/issues?q=is%3Aopen+is%3Aissue+label%3Asupport).
...
* Be kind and patient with users -- often, folks who have run into confusing things might be upset or impatient. This is okay. Try to understand where they're coming from, and if you're too uncomfortable with the tone, feel free to stay away or withdraw from the issue. (Note: if the user is outright hostile or is violating the CoC, [refer to the Code of Conduct](docs/code_of_conduct.md) to resolve the conflict).
...
In order to label issues, [open up the list of unlabeled issues](https://github.com/standup-raven/standup-raven/issues?q=is%3Aopen+is%3Aissue+no%3Alabel) and, **from newest to oldest**, read through each one and apply issue labels according to the table below. If you're unsure about what label to apply, skip the issue and try the next one: don't feel obligated to label each and every issue yourself!
...
`help wanted` | Applied by [Committers](#join-the-project-team) to issues and PRs that they would like to get outside help for. Generally, this means it's a lower priority for the maintainer team to implement itself, but that the community is encouraged to pick it up if they so desire | Never applied on first-pass labeling.
`in-progress` | Applied by [Committers](#join-the-project-team) to PRs that are pending some work before they're ready for review. | The original PR submitter should @mention the team member who applied the label once the PR is complete.
...
`wontfix` | Labelers may apply this label to issues that clearly have nothing at all to do with the project or are otherwise entirely outside of its scope/sphere of influence. [Committers](#join-the-project-team) may apply this label and close an issue or PR if they decide to pass on an otherwise relevant issue. | The issue or PR should be closed as soon as the label is applied, and a clear explanation of why the label was used should be provided. Contributors are free to contest the labeling, but the decision ultimately falls on the committers as to whether to accept something or not.
...
* Leave a message when closing saying "Cleaning up stale issue. Please reopen or ping us if and when you're ready to resume this. See [this](#clean-up-issues-and-prs) for more details."
...
There are many ways to contribute! Most of them don't require any official status unless otherwise noted. That said, there are a couple of positions that grant special repository abilities, and this section describes how they're granted and what they do.
